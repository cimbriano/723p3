...
class Chart:
    ...
    def iter_cell(self, i, j):
        if not self.chart.has_key(i): return
        if not self.chart[i].has_key(j): return
        for item in self.chart[i][j].itervalues():
            yield item
    ...
    def remove(self, i, j, label):
        if not self.chart.has_key(i): return
        if not self.chart[i].has_key(j): return
        if not self.chart[i][j].has_key(label): return
        del self.chart[i][j][label]

    def add(self, item):
        i = item.startPos
        j = item.endPos
        if not self.chart.has_key(i):
            self.chart[i] = {}
            self.greatestLogProb[i] = {}
        if not self.chart[i].has_key(j):
            self.chart[i][j] = {}
            self.greatestLogProb[i][j] = float('-inf')
            
        # only add if we don't already have a better version
        shouldAdd = True
        if self.chart[i][j].has_key(item.label):
